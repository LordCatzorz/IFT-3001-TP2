...
\begin{deriv}
  C^{D}\pars{n, m, x, y} 
  \<=
  \sum\limits_{i=0}^{\min(x, y)} \pars{m \cdot n}
  \<=
  \pars{m \cdot n} \cdot \sum\limits_{i=0}^{\min(x, y)} 1
  \<=
  \pars{m \cdot n} \cdot \min(x, y)
  \<\leq
  \commentaire{Puisque nous cherchons une borne supérieure polynomial,\\
              nous pouvons nous permettre d'abuser de l'inéquation. 
              $\min(x,y) \leq x * y \forall x, y \geq 0$}
  m \cdot n \cdot x \cdot y
  \<\in
  \BigO\pars{m \cdot n \cdot x \cdot y}
\end{deriv}
